...
vx_bool fcw_ttc_update(
    const fcw_ttc_data_t *d,
    fcw_ttc_manager_t *m
)
{
    fcw_ttc_track_t *t;
...
vx_uint32 fcw_ttc_get_history_length(
    const fcw_ttc_manager_t *m,
    vx_int32 id
)
{
...
)
{
    ...
    vx_float32 time[10],inv[10],mt=0.0F,mh=0.0F,st=0.0F,sh=0.0F,
               slope,intercept,res=0.0F,total=0.0F,r2;
    vx_uint32 i;
    ...
    for(i=0U;i<10U;i++)
    {
        ...
        time[i]=(vx_float32)(
            t->history.frame_id[i]-t->history.frame_id[9]
        )/(vx_float32)fps;
