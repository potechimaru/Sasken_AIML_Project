...
vx_status fcw_main_pre_init(FcwMainPreContext *x, const FcwMainPreConfig *c, FcwAlarmOutputFunc o,
                            void *u) {
    FcwMainPreConfig d;
    vx_status s;
    if (!x)
    ...
    x->config = c ? *c : d;
    s = fcw_tracker_init(&x->tracker, x->config.tracker_iou_threshold,
                         x->config.tracker_max_missed_frames);
    if (s != VX_SUCCESS)
        return s;
    s = fcw_alert_init(&x->alert_controller, x->config.alert_on_ttc_sec,
                       x->config.alert_off_ttc_sec, x->config.alert_required_count);
...
vx_status fcw_main_pre_begin_frame(FcwMainPreContext *x, vx_int32 f) {
    return x ? fcw_frame_result_reset(&x->frame_result, f) : VX_ERROR_INVALID_PARAMETERS;
}
vx_status fcw_main_pre_add_detection(FcwMainPreContext *x, const FcwDetection *d) {
    FcwCar *c;
    vx_float32 v;
    if (!x || !d)
        return VX_ERROR_INVALID_PARAMETERS;
    if (d->score < x->config.score_threshold || d->class_id != x->config.car_class_id)
        return VX_SUCCESS;
...
vx_status fcw_main_pre_add_tidl_channel(FcwMainPreContext *x, const sTIDL_IOBufDesc_t *io,
                                        vx_object_array out, vx_uint32 ch) {
    FcwDetection d[FCW_MAX_DETECTIONS];
    vx_uint32 n = 0U, i;
    vx_status s;
    if (!x)
        return VX_ERROR_INVALID_PARAMETERS;
    s = fcw_tidl_adapter_extract(io, out, ch, d, FCW_MAX_DETECTIONS, &n);
    if (s != VX_SUCCESS)
        return s;
    for (i = 0U; i < n; i++) {
...
vx_status fcw_main_pre_finalize_frame(FcwMainPreContext *x) {
    ...
    if (!x)
        return VX_ERROR_INVALID_PARAMETERS;
    s = fcw_tracker_update(&x->tracker, x->frame_result.cars, x->frame_result.num_cars, e,
                           FCW_MAX_TRACKS, &ec);
    if (s != VX_SUCCESS)
        return s;
    for (i = 0U; i < ec; i++) {
    ...
    for (i = 0U; i < x->frame_result.num_cars; i++) {
        ...
        if (fcw_ttc_update(&d, &x->ttc_manager)) {
            c->history_length = fcw_ttc_get_history_length(&x->ttc_manager, c->track_id);
            (void)fcw_ttc_calculate(x->config.fps, &x->ttc_manager, c->track_id, c);
        }
        (void)fcw_alert_update(&x->alert_controller, c);
    }
    s = fcw_alarm_update(&x->alarm, x->frame_result.cars, x->frame_result.num_cars);
    x->frame_result.any_alert = x->alarm.active;
    return s;
}
vx_status fcw_main_pre_process_tidl_frame(FcwMainPreContext *x, const sTIDL_IOBufDesc_t *io,
                                          vx_object_array out, vx_uint32 channels, vx_int32 frame) {
    vx_uint32 ch;
    vx_status s;
...
const FcwFrameResult *fcw_main_pre_get_frame_result(const FcwMainPreContext *x) {
