...
)
{
    ...
    vx_size start[4U]; //change APP_MAX_TENSOR_DIMS -> 4
    vx_size output_sizes[4U];
    vx_size output_strides[4U];
    ...
    (vxGetStatus((vx_reference)output_tensor) != VX_SUCCESS))
    {
...
)
{
    ...
    if ((vx_size)channel >= num_items)
    {
        return VX_ERROR_INVALID_PARAMETERS;
    }

    tensor = (vx_tensor)vxGetObjectArrayItem(
        output_tensor_arr, channel);
    ...
    status = fcw_tidl_adapter_extract_tensor(
        io_buf_desc,
        tensor,
        channel,
        detections,
    ...
    release_status = vxReleaseTensor(&tensor);

    return (status == VX_SUCCESS) ? release_status : status;
}